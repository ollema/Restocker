...
function events:BANKFRAME_OPENED(event, ...)
  ...
  --[[
    ...
    local bank = {}
    local inventory = {}
    local itemsRestocked = {}
    local restockedItems = false
    ...
    for k, bag in pairs(bankBags) do
      ...
      for slot = 1, GetContainerNumSlots(bag) do
        bank[bag][slot] = {}
        local _, stackSize, _, _, _, _, itemLink, _, _, itemID = GetContainerItemInfo(bag, slot)
        if itemLink ~= nil then
          local itemName, itemLink, _, _, _, _, _, itemStackCount = GetItemInfo(itemLink)
          bank[bag][slot].itemID = itemID
          bank[bag][slot].itemName = itemName
          bank[bag][slot].itemLink = itemLink
          bank[bag][slot].stackSize = stackSize
          bank[bag][slot].bag = bag
          bank[bag][slot].slot = slot
          bank[bag][slot].maxStack = itemStackCount
    ...
    for bag = 0, NUM_BAG_SLOTS do
      ...
      for slot = 1, GetContainerNumSlots(bag) do
        ...
        local _, stackSize, _, _, _, _, itemLink, _, _, itemID = GetContainerItemInfo(bag, slot)
        if itemID ~= nil then
          local itemName, itemLink, _, _, _, _, _, itemStackCount = GetItemInfo(itemID)

          inventory[bag][slot].itemID = itemID
          inventory[bag][slot].itemName = itemName
          inventory[bag][slot].itemLink = itemLink
          inventory[bag][slot].stackSize = stackSize
          inventory[bag][slot].bag = bag
          inventory[bag][slot].slot = slot
          inventory[bag][slot].maxStack = itemStackCount
        end
    ...
    -- POISON REAGENTS
    for itemName, amount in pairs(poisonReagentsNeeded) do
      itemsRestocked[itemName] = 0

      local numItemsInBags = GetItemCount(itemName, false)
      local numItemsInBank = GetItemCount(itemName, true) - numItemsInBags
      local restockNum = amount
      local difference = restockNum-numItemsInBags

      _, _, _, _, _, _, _, itemStackCount = GetItemInfo(itemName)

      if difference > 0 then

        if difference >= numItemsInBank then
          for bankbag, bankbagtable in pairs(bank) do
            for _, bankslot in ipairs(bank[bankbag]) do
              if bankslot.itemName == itemName then
                UseContainerItem(bankslot.bag, bankslot.slot)
                itemsRestocked[itemName] = itemsRestocked[itemName] + bankslot.stackSize
                restockedItems = true
                bankslot = nil
              end
            end
          ...
          for bankbag, bankbagtable in pairs(bank) do
            for _, bankslot in ipairs(bank[bankbag]) do
              if bankslot.itemName == itemName and difference > 0 then

                if difference >= bankslot.stackSize then
                  UseContainerItem(bankslot.bag, bankslot.slot)
                  itemsRestocked[itemName] = itemsRestocked[itemName] + bankslot.stackSize
                  restockedItems = true
                  difference = difference-bankslot.stackSize
                  bankslot = nil


                else
                  if bankslot.stackSize > difference then
                    SplitContainerItem(bankslot.bag, bankslot.slot, difference)

                    for invbag, invbagtable in pairs(inventory) do
                      for _, invslot in ipairs(inventory[invbag]) do
                        if invslot.itemName == itemName and (invslot.stackSize + difference == invslot.maxStack) then
                          PickupContainerItem(invslot.bag, invslot.slot)
                          itemsRestocked[itemName] = itemsRestocked[itemName] + difference
                          restockedItems = true
                          invslot.stackSize = invslot.stackSize+difference
                        end
                      end
                    end

                    difference = difference - difference
                    bankslot.stackSize = bankslot.stackSize - difference
                  else
                    UseContainerItem(bankslot.bag, bankslot.slot)
                    itemsRestocked[itemName] = itemsRestocked[itemName] + bankslot.stackSize
                    restockedItems = true
          ...
          for bankbag, bankbagtable in pairs(bank) do
            for _, bankslot in ipairs(bank[bankbag]) do
              if bankslot.itemName == itemName and difference > 0 then
                ...
                if difference > bankslot.stackSize or difference == bankslot.stackSize then
                  UseContainerItem(bankslot.bag, bankslot.slot)
                  itemsRestocked[itemName] = itemsRestocked[itemName] + bankslot.stackSize
                  restockedItems = true
                  numItemsInBags = numItemsInBags + bankslot.stackSize
                  difference = difference-bankslot.stackSize
                  bankslot = nil
                    ...
                    for invbag, invbagtable in pairs(inventory) do
                      for _, invslot in ipairs(inventory[invbag]) do
                        if invslot.itemName == itemName and (invslot.stackSize + difference <= invslot.maxStack) then
                          ...
                          restockedItems = true
                          invslot.stackSize = invslot.stackSize+difference
                          numItemsInBags = numItemsInBags + difference
                          difference = difference - difference
                          bankslot.stackSize = bankslot.stackSize - difference
                        end
                      end
    ...
    if restockedItems then
      core:Print("Restocked the following items")
      for k, v in pairs(itemsRestocked) do
        if v > 0 then
          core:Print("  "..k.." x"..v)
        end
      end
